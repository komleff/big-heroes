...
- `APPROVED` от Claude + `APPROVED` от обоих внешних ревьюеров (GPT-5.4, GPT-5.3-Codex) → можно мержить
- В fallback-режиме (одна модель, два прохода): `APPROVED` от Claude + `APPROVED` от обоих проходов → можно мержить. Отчёт явно указывает режим.
