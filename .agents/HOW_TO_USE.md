...
Copilot re-review запрашивается автоматически после каждого fix-цикла.
...
- `APPROVED` от Claude + `APPROVED` от обоих внешних ревьюеров → можно мержить
- В fallback-режиме (одна модель, два прохода): `APPROVED` от обоих проходов → можно мержить. Отчёт явно указывает режим и модели.
- `CHANGES_REQUESTED` → PM сам исправит через Developer, скажи «устрани замечания»
- Если аспект помечен как «не проверен» (модель не покрыла его) → PM запускает повторный `/external-review`
