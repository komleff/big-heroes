...
**Решение:** Sprint Final выполняется автоматически скиллом `/external-review`. Два внешних ревьюера работают последовательно по **всем 4 аспектам**. В API key режиме: GPT-5.4 + GPT-5.3-Codex. В ChatGPT login режиме: дефолтная модель, два прохода с разным фокусом. Оба ревьюера проверяют все 4 аспекта (архитектура, безопасность, качество, гигиена кода). Claude PM консолидирует результаты и публикует в PR. Copilot re-review запрашивается автоматически после каждого fix-цикла.
