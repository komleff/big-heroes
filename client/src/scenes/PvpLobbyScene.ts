...
export class PvpLobbyScene extends BaseScene {
    ...
    onEnter(): void {
        ...
        // --- Нижние кнопки ---
        // Активная сессия → только «Завершить сессию» (manual end, переход через оверлей в Hub с clearArenaSession).
        // Нет сессии → «← Назад в Хаб» (старое поведение, обычный back-переход).
        // Выход в Hub без завершения сессии намеренно не предусмотрен — серия боёв должна быть явно закрыта.
        let btnY = nextY + 16;
        if (session && !preCheck.ended) {
