...
### Определение стадии landing (pre vs post)

С v3.4 skill вызывается дважды (см. Dual-invocation pattern выше). Первый
вызов — до landing commit; второй — после. В финальный комментарий добавляется
соответствующий маркер, чтобы оператор не смержил PR между первым и вторым
вызовом (митигация R4 из плана `sprint-pipeline-v3-4-pre-merge-landing`):

- **Первый вызов** (landing commit ещё не сделан): добавить строку
  `⏳ Pre-merge landing commit впереди — жди второй /finalize-pr, не мерджи сейчас.`
...
Переменная `LANDING_WARNING` подставляется в шаблоны фазы 1 и 2 через bash
parameter expansion `${LANDING_WARNING:+...}` — если пусто, строка не
печатается; если непусто, предупреждение добавляется отдельным абзацем.
