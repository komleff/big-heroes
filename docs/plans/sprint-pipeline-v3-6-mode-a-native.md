...
- Выход: markdown на stdout в фиксированном формате из четырёх секций (Архитектура, Безопасность, Качество, Гигиена кода) и строкой `## Вердикт: <APPROVED | CHANGES_REQUESTED | ESCALATION>`. Скрипт форсирует контракт через system prompt и пост-валидацию структуры ответа (`validateOutputFormat`); ручного маппинга PM на аспекты при консолидации не требуется — PM публикует raw и пишет свою PM-сводку по уже нормализованному ответу.
...
- E4. Diff пустой (нет изменений относительно base-ветки PR) → exit с сообщением «nothing to review», не отправка пустого prompt в API. **Контракт `--base`:** в `openai-review.mjs` передаётся именно имя ветки (значение `baseRefName` из `gh pr view`, например `main`), а не готовый git-ref. Скрипт сам перед сравнением делает `git fetch` с **явным refspec** и сравнивает `HEAD` с `origin/<baseRefName>`. Критерий в коде: `BASE_REF=$(gh pr view --json baseRefName --jq '.baseRefName'); git fetch origin "+refs/heads/$BASE_REF:refs/remotes/origin/$BASE_REF"; git diff --quiet "origin/$BASE_REF...HEAD"` (exit 0 = нет изменений). **Почему явный refspec:** простая форма `git fetch origin "$BASE_REF"` обновляет только `FETCH_HEAD`, но **не гарантированно** обновляет `refs/remotes/origin/$BASE_REF` — это зависит от пользовательской настройки `remote.origin.fetch` в `.git/config`. Явный refspec `+refs/heads/<branch>:refs/remotes/origin/<branch>` гарантирует force-update именно того ref, с которым сравниваем. **Почему `origin/<baseRefName>`:** локальный ref `$BASE_REF` может не существовать или быть неактуальным (PR из fork/feature-base).
